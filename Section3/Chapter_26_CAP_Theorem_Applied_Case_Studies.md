...
═══════════════════════════════════════════════════════════
  ...
  ✓ All nodes in sync   ✓ All requests served   ✓ Network healthy
...
=====================================
  ...
  Total allowed: 100 × 5 = 500 requests
  Intended limit: 100 requests

  Attacker gets 5× the allowed rate. During a partition.
  If partition lasts 5 minutes → 5 × 500 = 2,500 requests.
```
...
===================================================
  ...
  │ Reality: You're 2× over your limit          │
  └─────────────────────────────────────────────┘
```
...
──────────────────────┼───────────────────────┼─────────────────────────
Abusive user,         │ Gets blocked (correct) │ May get 2–5× limit if
...
========================
  ...
  Typical case (traffic spread across 2-3 regions):
    Total allowed = 10 × 2 = 20 to 10 × 3 = 30
...
── AP CHOICE ────────────────────────────────────────────────────────────
    ...
    │ morning vibes☕" │             │ into the sunrise"│
    │ 9:01am           │             │ 9:02am           │
    │                  │             │                  │
    │ Diana: "Coffee   │             │ Carlos: "Monday  │
    │ first, email     │             │ morning vibes☕" │
...
┌─────────────────────────────────────────────────────────┐
│  8:55am  Carlos: "Coffee first"          ✓ both agree   │
│  8:58am  Diana: "Good morning"           ✓ both agree   │
│  9:00am  Eve: "Starting my day"          ✓ both agree   │
...
- **Pending (⏳):** In device queue. Not yet sent. AP — app accepts it locally.
- **Sent (✓):** Our server confirmed receipt. AP — your region got it.
- **Delivered (✓✓):** Recipient's device acknowledged. CP-ish — we wait for confirmation.
- **Read (blue ✓✓):** Recipient opened conversation. CP — only shown when confirmed.
...
**During a partition:** The message stays at ✓ "Sent" for longer. The UI is honest. Users understand "the message is out there, delivery pending." This is much better than either showing a fake "Delivered" (lying) or showing "Failed" (confusing).

**Never lie in status indicators.** Showing ✓✓ when you haven't confirmed delivery is a trust violation. When the user asks Bob "did you get my message?" and he says "no" but the app shows delivered — that's a relationship and trust catastrophe.
...
T+0:00  Partition starts
        │
        │  US-East ──✗── US-West (network cut)
        │
T+0:01  Checkout requests flood in
        │
        │  ┌──────────────────────────────────────────────────────┐
        │  │  User 1: "Buy PS5"  →  ❌ Service Unavailable        │
        │  │  User 2: "Buy PS5"  →  ❌ Service Unavailable        │
        │  │  User 3: "Buy PS5"  →  ❌ Service Unavailable        │
        │  │  ...                                                  │
        │  │  User 500: "Buy PS5"→  ❌ Service Unavailable        │
        │  └──────────────────────────────────────────────────────┘
        │
...
2. Status indicators (⏳ ✓ ✓✓) are the UI contract — they make AP feel trustworthy.
...
```
  ...
  ✓ No conflict. Order of merge does not matter. Always correct.
```
...
──────────────────────────────
  ...
  Affected per event: 500,000,000 × 0.01% = 50,000 users
...
Think through: each region's rate limiter enforces limits locally, based on local counters. The attacker sends N requests per second to each of 3 regions. Each region allows up to your per-user limit because it does not know about the other regions' counts. Total throughput = 3× your intended rate limit. For 10 minutes. Is your backend designed to handle 3× traffic? What does the SLA say? Who pays for the compute? Is the AP rate limiter saving you money in normal operation, and costing you more during the rare partition?
...
│  "Rare × scale = regular. Plan for partitions before they happen."      │
└─────────────────────────────────────────────────────────────────────────┘
```
