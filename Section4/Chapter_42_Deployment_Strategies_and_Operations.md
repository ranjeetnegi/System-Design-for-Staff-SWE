...
### Stratified canary sampling: the Facebook iOS lesson

A canary is only as good as its sample. In 2012, Facebook deployed a new iOS app
version with a canary at 1% of users. The bug only triggered on iOS 5.0 devices.
iOS 5.0 was 8% of their user base, but their canary pool was mostly beta testers
on iOS 6. The canary showed 0 errors. They promoted to 100%. 40 million iOS 5.0
users saw crashes.

The lesson: if your users are heterogeneous (different OS versions, device types,
geographies, languages), a random 1% sample may not represent them.

```
  THE STRATIFICATION PROBLEM:

  Facebook's canary (1% random sample):
    90% iOS 6 users (no bug)
    10% iOS 5.x users (bug on 5.0 only, not 5.1)
    iOS 5.0 in canary = 0.1% of 1% = 0.001% of total users
    At small canary size: 0 crashes observed

  After full rollout (8% of users on iOS 5.0 = 40M users):
    All 40M crash

  FIX: stratified sampling
    Canary pool must MATCH production distribution:
    - 8% of canary users must be on iOS 5.x
    - Geographic distribution must match
    - Device type distribution must match

  Verify: for each segment in your canary, the sample size must be
  large enough to detect a 1% error rate with statistical significance.
```

**Staff lesson:** define your canary SLIs per segment (OS version, geography,
device type), not just in aggregate. An aggregate 0% error rate can hide a 100%
error rate in a specific user segment.
...
Blue-green works perfectly for stateless HTTP APIs. It breaks for services with
long-lived connections (WebSockets, gRPC streams, database connection pools).

Imagine a chat service with 500,000 WebSocket connections on Blue. You flip
traffic to Green. What happens to those connections?

```
  BLUE-GREEN WITH WEBSOCKETS:

  Before flip:
    Blue:  500K WebSocket connections (active)
    Green: 0 connections (warmed up, no users)

  Traffic flip (load balancer):
    All NEW connections -> Green
    Existing connections -> still on Blue (the connection is pinned)

  Problem: Blue must stay alive until all 500K connections naturally close.
  That could take hours. You now have BOTH environments serving traffic.
  "Blue-green" becomes "blue-and-green" until the drain completes.

  Options:
  a) Force-close connections on Blue: users get disconnected (bad UX)
  b) Drain Blue slowly: keep Blue up, wait hours for natural reconnects
  c) Don't use blue-green for WebSocket services: use rolling deploy instead
```

**Staff lesson:** choose deployment strategy based on connection type. Blue-green
for stateless HTTP APIs. Rolling deploy for WebSocket/gRPC services (keeps some
old pods alive to drain existing connections naturally).
...
### High cardinality: why you cannot use user_id as a metric label

Metrics work by labeling data points. Good labels: `service`, `endpoint`,
`status_code`, `region`. These have low cardinality (dozens of unique values).

A common mistake is adding high-cardinality labels like `user_id` or
`request_id`. Here is why this crashes monitoring systems:

```
  LOW CARDINALITY (fine):
    http_requests_total{service="payment", status="200"} = 1,500,000
    http_requests_total{service="payment", status="500"} = 1,200
    Unique time series: ~10 (a few services x a few status codes)

  HIGH CARDINALITY (dangerous):
    http_requests_total{user_id="usr_0000001", status="200"} = 5
    http_requests_total{user_id="usr_0000002", status="200"} = 12
    ... for 10 million users ...
    Unique time series: 10,000,000+

  Prometheus with 10M time series:
    - Memory: 10M x ~3KB per series = 30GB just for this one metric
    - Query time: seconds to minutes
    - Likely: OOM crash, Prometheus down, no monitoring
```

The fix: use high-cardinality data for logs and traces (where you look up
one request at a time). Use low-cardinality data for metrics (where you
aggregate across millions of requests). Never use user_id, request_id,
session_id, or IP addresses as metric labels.

### The content SLI trap: HTTP 200 does not mean success

Google's 2021 incident shows a dangerous SLI design mistake. Their service
returned HTTP 200 with an empty response body `{}` when it was broken. The SLI
(measured as fraction of HTTP 200 responses) showed 99.99% availability. The
service was actually broken for 4 hours. Users saw blank pages.

```
  STATUS-ONLY SLI (broken):
    SLI = count(HTTP 200) / count(all requests)
    Broken service returns 200 with body: {}
    SLI = 99.99%  <-- "all good" (wrong)

  CONTENT SLI (correct):
    SLI = count(requests where: HTTP 200
                AND body contains required fields
                AND response size > 100 bytes)
              / count(all requests)
    Broken service: body is {}, size = 2 bytes -> fails content check
    SLI = 0%  <-- "broken" (correct)
```

Design SLIs that measure user-observable correctness, not just server response
codes. Ask: "If my service returned empty responses for every request, would
my SLI catch it?"
...
### The auto-scaling lag problem

Auto-scaling adds capacity when load spikes. But adding a new pod takes time:
pull the container image, start the process, warm up caches, pass health checks.
This is typically 2-5 minutes. A traffic spike that doubles in 90 seconds will
overwhelm your current capacity before auto-scaling can help.

```
  AUTO-SCALING LAG SCENARIO:

  T=0:00  Traffic: 10K req/sec  Pods: 20  CPU: 60%
  T=0:30  Traffic: 15K req/sec  Pods: 20  CPU: 90%  <- alert fires
  T=1:00  Traffic: 20K req/sec  Pods: 20  CPU: 120% <- requests queuing
  T=1:30  Traffic: 25K req/sec  Pods: 20  CPU: 150% <- pods OOM, crash
  T=2:00  Auto-scaler triggers: add 10 pods
  T=4:00  New pods ready (2 min to start)
  T=4:30  Pods: 30, Traffic: 25K req/sec, CPU: 70% <- recovered
           But: 2.5 minutes of degradation already happened

  GAP: 90 seconds to detect + 120 seconds to provision = 3.5 min lag
  During those 3.5 minutes: users experience failure
```

Strategies to close the gap:
- **Predictive scaling:** pre-scale before known traffic events (Monday 9 AM,
  end-of-month batch, marketing campaign launches)
- **Over-provision baseline:** run at 40% capacity instead of 70%, giving more
  headroom before the cliff
- **Faster pod startup:** optimize container image size, pre-pull images,
  reduce health check wait time
- **Horizontal pod autoscaler tuning:** lower the CPU threshold that triggers
  scaling (e.g., 60% instead of 80%)

**Staff lesson:** capacity planning is not just "add auto-scaling." It is
designing the headroom, startup time, and trigger thresholds so that the
combination responds faster than your traffic can spike.
...
### The database rollback trap: Etsy 2012

Before covering rollback strategies, understand the most dangerous rollback
scenario: rolling back code when a database migration has already run.

In 2012, Etsy deployed a new feature that added a database column. Twenty
minutes post-deploy, they found a pricing bug and rolled back the code. But
they had not written a rollback script for the migration. The old code was
back -- but the new column existed in the database. Old code did not write
to the column. Some rows now had NULL where the new code expected a value.
Items showed $0.00 to buyers. A different bug caused by the rollback.

```
  ETSY'S ROLLBACK FAILURE:

  Deploy: add column "price_v2" (nullable) + new code writes to it
  Bug found: pricing calculation wrong
  Action: roll back code to old version

  Old code: does not know about price_v2 column
  But: some rows already have price_v2 = NULL (written by new code)
  Old code logic: if price = 0, use fallback -> reads NULL as 0
  Result: items show $0.00 to buyers

  Rollback created a DIFFERENT bug, not a clean restore.

  ROOT CAUSE: no rollback migration script written.
  The forward migration existed. The reverse migration did not.
```

**The rule:** every database migration must have a corresponding rollback
script, written and tested at the same time as the forward script. Run both
in staging before production. A rollback you have never tested is not a
rollback plan -- it is optimism.

For schema changes specifically, use the 3-phase pattern:
1. Add column as NULLABLE (backward compatible -- both old and new code work)
2. Deploy new code (reads and writes the column)
3. Only then add NOT NULL constraint (safe because all rows have the value)

This way, rolling back code (step 2) never creates an inconsistency, because
the schema change (step 1) is backward compatible with the old code.
...
*Same problem at four levels: your team needs to deploy a new version of the
payment service. The new version changes the database schema (adds a NOT NULL
column). Any downtime or data corruption means lost revenue and potential
chargebacks.*

---

### Intern Level: "Deploy at 3 AM when traffic is low"

The intern schedules the deployment for 3 AM Sunday. Low traffic = fewer users
affected if it breaks.

```
  Intern's plan:
  +------------------------------------------+
  | 1. Wait for Sunday 3 AM (low traffic)    |
  | 2. Run database migration (ALTER TABLE)  |
...
What goes wrong: 3 AM means no senior engineers awake. No one to help if it
breaks. The ALTER TABLE on a 50-million-row table locks the table for 12 minutes.
During those 12 minutes, every payment fails. $180K in lost transactions. The
intern wakes up their manager at 3:15 AM.

The core mistake: treating "low traffic" as a safety substitute for "good
deployment practice." Low traffic just means fewer people witness the failure.

---

### Mid-Level (L4): "Use blue-green, swap traffic instantly"

L4 knows about blue-green. They spin up a Green environment with the new version,
test it, then switch all traffic from Blue to Green in one step.

```
  L4's blue-green plan:
  +-------+         +-------+
  | BLUE  |         | GREEN |
  | old   |  --->   | new   |
  | code  |  swap   | code  |
...
This solves the downtime problem. Traffic switches in seconds. If something
breaks, swap back.

But L4 missed one thing: the database. Both Blue and Green share the same
database. The new version adds `currency VARCHAR NOT NULL`. L4 runs the migration
before the swap. Now the column exists. Blue (old code) is still running during
the migration window. Old code does INSERT without the `currency` column. NOT NULL
constraint fires. Payments fail during the migration window, before the swap even
happens.

L4 solved the code deployment problem but broke on the database coupling.

---

### Senior (L5): "Decouple schema migration from code deployment"

L5 recognizes that schema changes and code changes must be deployed in separate
steps. Doing them together is the root cause of L4's failure.

```
  L5's 3-phase plan:

  Phase 1 -- Schema migration (backward compatible):
    ALTER TABLE payments ADD COLUMN currency VARCHAR(3) NULL;
    -- NULL, not NOT NULL. Old code still works fine. No breakage.
    Deploy this. Wait 24 hours. Verify.

  Phase 2 -- Code deployment (canary):
    New code runs on 1% of traffic.
    Monitor for 24 hours:
      - payment_success_rate >= 99.5%?    [Y] promote
      - p99 latency < 200ms?              [Y] promote
      - currency column populated?        [Y] promote
    1% -> 10% -> 50% -> 100% over 3 days.

  Phase 3 -- Schema cleanup (after 100% on new code):
    ALTER TABLE payments ALTER COLUMN currency SET NOT NULL;
    Safe now: all writes for the past 3 days have currency set.
```

L5 also defines: who monitors the canary, what metric triggers rollback, who
approves each promotion step.

Result: zero downtime, zero data errors, 3-day controlled rollout.

---

### Staff (L6): "Deployment is a program, not a task"

L6 does everything L5 proposes, then asks the questions no one else asked.

**Question 1: Who is affected at each canary percentage?**
"At 1% canary, we are exposing 5,000 payments/hour to the new code. Our merchant
SLA guarantees 99.9% uptime monthly -- a 30-minute degradation at 1% still uses
4% of our monthly error budget. The canary window must be short enough that a
rollback does not breach the SLA."

**Question 2: What does 'done' mean?**
"The deployment is not done when kubectl apply finishes. It is done when: (a) 100%
traffic on new version, (b) monitored for 48 hours, (c) settlement reports for
3 days verified correct, (d) old Blue environment decommissioned, (e) runbook
updated with anything we learned."

**Question 3: Who can roll back, and how fast?**
"The on-call engineer has rollback authority at any canary stage. No approval
needed. They must file an incident ticket within 15 minutes. This removes decision
paralysis: no one should be calling their manager at 2 AM to ask permission to
roll back a failing canary."

```
  L6's pre/during/post checklist:

  PRE-DEPLOYMENT:
  [ ] Phase 1 migration tested on production-size dataset (timing verified)
  [ ] Canary success criteria written down (not "looks good")
  [ ] Rollback plan tested in staging (not just written)
  [ ] Merchant status page message drafted
  [ ] On-call engineer briefed; has rollback command ready

  DURING DEPLOYMENT:
  [ ] SLI dashboard live: payment_success_rate, p99, currency_populated_rate
  [ ] Each canary stage has a time gate (not just a metric gate)
  [ ] On-call has rollback authority; no approval needed

  POST-DEPLOYMENT:
  [ ] 48-hour observation window
  [ ] Settlement reports for 3 days verified
  [ ] Old Blue environment decommissioned (cost cleanup)
  [ ] Runbook updated with surprises encountered
  [ ] Error budget impact recorded in the SLO tracker
```
...
```
  Intern:  deploy at 3 AM, no rollback plan
           -> problem: no support, long migration locks table

  L4:      blue-green, instant swap
           -> problem: schema and code deployed together, data conflict

  L5:      3-phase (schema NULL -> canary -> schema NOT NULL)
           -> solves coupling; defines SLIs and rollback triggers

  L6:      deployment as a program with checklist, rollback authority
           policy, error budget awareness, merchant SLA tracking
           -> deployment is finished when business verifies it, not
              when the pipeline says green
```

The jump from L5 to L6 is not technical knowledge. L5 knows all the same
techniques. The jump is: L6 thinks about deployment in terms of business
commitments, organizational authority, and total-cost-of-done. L5 thinks about
deployment in terms of technical steps.
