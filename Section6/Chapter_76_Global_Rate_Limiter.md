# Chapter 55: Global Rate Limiter
...
# Appendix: DDoS and Abuse Mitigation — Staff-Level Depth

Rate limiting protects against overuse. DDoS and abuse mitigation protects against intentional attack. At Staff level, you're expected to distinguish volumetric attacks from application-layer abuse and design layered defenses.

**The Staff Engineer's DDoS Principle**: Volumetric attacks are infrastructure problems (solved with money). L7 attacks are engineering problems (solved with code). Rate limiting handles fair use; DDoS mitigation handles survival. Design for both.

**One-liners**:
- "If you can solve it with Cloudflare, it's not your problem. If Cloudflare can't see it, it is."
- "The hardest DDoS to stop looks exactly like your best customer."
- "Defense in depth means the attacker has to be right every time; you only need to block them once per layer."

## L5 vs L6: DDoS Thinking

| Scenario | L5 Approach | L6 Approach |
|----------|-------------|-------------|
| **DDoS protection** | "We have rate limiting" | "Rate limiting stops fair-use abuse. Volumetric DDoS saturates the pipe before requests reach our rate limiter. We need edge scrubbing (Cloudflare/Shield) for L3/L4, WAF for L7 patterns, rate limiting for per-user abuse, and application-level bot detection for sophisticated L7." |
| **Bot detection** | "Block suspicious IPs" | "IPs rotate. We fingerprint behavior: request timing patterns, header ordering, JS execution capability, mouse movement. Progressive challenge: no friction for humans, increasing friction for bots." |
| **Under attack** | "Block all traffic from that country" | "Geo-blocking is a blunt instrument — 30% of our users are in that country. Instead: rate limit by IP + behavioral scoring + CAPTCHA for suspicious patterns. Monitor false positive rate. Accept 5% revenue loss from friction vs 100% loss from outage." |
| **Cost of protection** | "DDoS protection is expensive" | "Cloudflare Pro: $20/month. A 1-hour outage costs $50K in revenue. The math isn't close. Edge protection is insurance with 2500× ROI on first incident." |
...
│                                                                             │
│   LAYER 3/4 — VOLUMETRIC (flood the pipe):                                 │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  • SYN flood: exhaust connection table                              │   │
│   │  • UDP flood: saturate bandwidth                                    │   │
│   │  • Amplification (DNS, NTP): small request → large response         │   │
│   │  • Volume: 100 Gbps – 3+ Tbps                                      │   │
│   │  • Defense: edge scrubbing (Cloudflare, AWS Shield), anycast       │   │
│   │  • Your app NEVER sees this traffic — mitigated at network edge    │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│   LAYER 7 — APPLICATION (abuse the app):                                    │
│   ┌─────────────────────────────────────────────────────────────────────┐   │
│   │  • Slowloris: hold connections open, exhaust server threads         │   │
│   │  • HTTP flood: valid-looking GET/POST at high volume                │   │
│   │  • API abuse: scraping, credential stuffing, enumeration            │   │
│   │  • Bot traffic: automated fake accounts, fake reviews              │   │
│   │  • Volume: 10K–1M req/sec (looks like real traffic)                │   │
│   │  • Defense: WAF rules, rate limiting, CAPTCHA, bot detection       │   │
│   │  • Your app DOES see this traffic — must distinguish from legit    │   │
│   └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│   KEY INSIGHT: Volumetric attacks are infrastructure problems               │
│   (solved with money: Cloudflare, AWS Shield). L7 attacks are              │
│   application problems (solved with engineering).                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
│        │                                                                    │
│        ▼                                                                    │
│   ┌─── LAYER 1: EDGE SCRUBBING (Cloudflare, AWS Shield, Akamai) ────────┐ │
│   │  Absorb L3/L4 volumetric (SYN, UDP, amplification)                   │ │
│   │  Anycast: distribute across global PoPs. Cost: $3K–$20K/month       │ │
│   │  BLOCKS: 95% of volumetric attacks. App never sees this traffic.    │ │
│   └──────────────────────────────┬───────────────────────────────────────┘ │
│        │ clean traffic           │                                         │
│        ▼                         │                                         │
│   ┌─── LAYER 2: CDN + WAF ──────┘──────────────────────────────────────┐  │
│   │  WAF rules: SQLi, XSS, known attack signatures                      │  │
│   │  Bot detection: JS challenge, fingerprinting, CAPTCHA                │  │
│   │  Geo-blocking: restrict if applicable                                │  │
│   │  BLOCKS: Known attack patterns, automated scanners                  │  │
│   └──────────────────────────────┬───────────────────────────────────────┘  │
│        │                         │                                         │
│        ▼                         │                                         │
│   ┌─── LAYER 3: API GATEWAY ────┘──────────────────────────────────────┐  │
│   │  Rate limiting per client/API key (THIS CHAPTER)                     │  │
│   │  Auth: reject unauthenticated early. Payload size limits.           │  │
│   │  BLOCKS: Per-user abuse, API key abuse                              │  │
│   └──────────────────────────────┬───────────────────────────────────────┘  │
│        │                         │                                         │
│        ▼                         │                                         │
│   ┌─── LAYER 4: APPLICATION ────┘──────────────────────────────────────┐  │
│   │  Business-logic abuse: fake accounts, credential stuffing           │  │
│   │  Progressive challenges: CAPTCHA after suspicious behavior          │  │
│   │  Anomaly detection: unusual patterns → review queue                 │  │
│   │  BLOCKS: Sophisticated L7 abuse that looks like real traffic        │  │
│   └──────────────────────────────┬───────────────────────────────────────┘  │
│        │                         │                                         │
│        ▼                         │                                         │
│   ┌─── LAYER 5: DATABASE ───────┘──────────────────────────────────────┐  │
│   │  Connection limits per service. Query timeouts. Read replicas.      │  │
│   │  BLOCKS: Resource exhaustion from queries that bypass upper layers  │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
│   STAFF PRINCIPLE: Each layer catches what the previous layer missed.      │
│   No single layer is sufficient. Together they're defense in depth.        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

## L7 Abuse Patterns and Mitigations

| Abuse Pattern | Detection | Mitigation | Blast Radius if Unmitigated |
|--------------|-----------|------------|----------------------------|
| **Credential stuffing** | High login failure rate from IP range | Rate limit login endpoint; CAPTCHA after 3 failures; notify user | Account takeover, trust loss |
| **Scraping** | High request rate to listing/search pages | Rate limit per session; require auth; vary HTML structure | Data theft, competitive loss |
| **Enumeration** | Sequential ID access (/users/1, /users/2, ...) | Use UUIDs not sequential IDs; rate limit | PII exposure, compliance violation |
| **Account creation spam** | Burst of signups from similar IPs/fingerprints | Email/phone verification; CAPTCHA; rate limit signup | Platform pollution, cost (storage, email) |
| **Inventory hoarding** | Bots holding items in cart without purchasing | Cart expiry timer; CAPTCHA at checkout; limit per session | Revenue loss, customer frustration |
| **API key abuse** | Legitimate key used far beyond expected volume | Per-key rate limits; usage alerts; progressive throttling | Cost explosion, downstream overload |

## Cost of DDoS Protection

| Layer | Tool | Monthly Cost | What It Stops |
|-------|------|-------------|---------------|
| Edge scrubbing | Cloudflare Pro | $20 | Basic L3/L4 + WAF |
| Edge scrubbing | Cloudflare Business | $200 | Advanced WAF, bot management |
| Edge scrubbing | AWS Shield Standard | Free (with ALB) | Basic L3/L4 |
| Edge scrubbing | AWS Shield Advanced | $3,000 | L3/L4/L7, DDoS response team, cost protection |
| Bot management | Cloudflare Bot Management | $500+ | Sophisticated L7 bots |
| WAF | AWS WAF | $5/rule + $0.60/M requests | L7 patterns |

**Staff cost insight**: A 1-hour outage from an unmitigated DDoS costs $50K–$500K for a mid-size SaaS company (revenue + SLA credits + incident response). Cloudflare Pro at $20/month has infinite ROI on the first attack. AWS Shield Advanced at $3K/month includes cost protection — AWS covers the bill for DDoS-induced scaling.

## Production Incident: Credential Stuffing at Scale

**Context**: E-commerce platform with 50M registered users. Login endpoint at `/api/v1/auth/login`.

**Attack**: Attacker used 100K compromised credential pairs (from a different breach) against the login endpoint. Requests came from 10K IPs globally at 5K req/sec — looked like normal login traffic.

**Impact**: 200 accounts compromised (0.0004%). But the real damage: login service scaled to handle 5K req/sec additional load, hitting database connection limits. Legitimate users couldn't log in for 45 minutes.

**What failed**: Rate limiting was per-IP (100 req/min). With 10K IPs, attacker had 1M req/min budget. No behavioral detection. No account lockout.

**Fix**: (1) Rate limit login by IP to 10/min (aggressive). (2) CAPTCHA after 3 failed attempts per account. (3) Credential breach detection: check passwords against known breached datasets (HaveIBeenPwned API). (4) Behavioral scoring: flag logins from new device + new IP + new location. (5) Alert on aggregate login failure rate, not just per-IP.

**Lesson**: Per-IP rate limiting fails against distributed attacks. Defend at multiple granularities: per-IP, per-account, and aggregate.

## Quick Reference Card: DDoS & Abuse

```
╔═══════════════════════════════════════════════════════════════════════════════╗
║        QUICK REFERENCE: DDoS & ABUSE — REMEMBER THIS                        ║
╠═══════════════════════════════════════════════════════════════════════════════╣
║                                                                               ║
║  TWO KINDS OF ATTACK:                                                         ║
║  L3/L4 Volumetric → Edge scrubbing (Cloudflare, Shield) → You never see it  ║
║  L7 Application   → WAF + Rate Limit + App Logic → You must engineer it      ║
║                                                                               ║
║  DEFENSE LAYERS:  Edge → CDN/WAF → Gateway → App → DB                        ║
║  (Each catches what the previous missed)                                     ║
║                                                                               ║
║  COST: Edge protection ($20-3K/mo) vs outage ($50K-500K/incident)            ║
║                                                                               ║
║  L7 ABUSE PLAYBOOK:                                                           ║
║  1. Rate limit at multiple granularities (IP + account + aggregate)          ║
║  2. Progressive challenge (CAPTCHA after suspicious, not every request)      ║
║  3. UUIDs not sequential IDs (prevent enumeration)                           ║
║  4. Monitor aggregate patterns, not just per-user                            ║
║                                                                               ║
║  ONE-LINER: "Volumetric is an infrastructure problem, L7 is your problem."  ║
║                                                                               ║
╚═══════════════════════════════════════════════════════════════════════════════╝
```
...
**When asked "How do you protect against DDoS?"** — "Layered defense. L3/L4 volumetric attacks: edge scrubbing (Cloudflare, AWS Shield)—our app never sees this traffic. L7 application attacks: WAF rules at the CDN, rate limiting at the API gateway, abuse detection in the application. Rate limiting handles fair use; DDoS mitigation handles survival. Key trade-off: aggressive blocking reduces attack impact but risks blocking legitimate users behind shared IPs. We use progressive challenges—CAPTCHA after suspicious behavior, not on every request."

**When asked "How do you handle credential stuffing?"** — "Multi-layered. Rate limit login by IP (10/min) and by account (5 failures → CAPTCHA). Check against known breached credential datasets. Behavioral scoring: new device + new IP + new location → step-up authentication. Monitor aggregate login failure rate—if it spikes 10× across all accounts, that's a coordinated attack, not individual failures."
