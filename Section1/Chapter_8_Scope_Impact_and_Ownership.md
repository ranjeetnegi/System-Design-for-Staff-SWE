...
New composite availability: message service × storage service only = 0.999 × 0.999 = 99.8%. Fan-out can fail independently without affecting the user's send success rate.
...
**L6 response:** "This is undifferentiated heavy lifting — work that does not differentiate any product but every team must do. Four teams × 20% = 0.8 full-time engineers spent on commodity work that creates no product value.
...
Question 1 — What breaks first as we scale 10×? No pagination means a single list endpoint request returns all records. At 10× traffic with 10× data, a single request returns 10 million records. The service runs out of memory. This is not a gradual degradation — it is a sudden cliff.
...
**L6 designs for today AND names the migration path to 10×.**
...
Now multiply this by 100 concurrent users. Each user triggers a B→C call. Each call occupies a thread in B for 10 seconds even though the user received an error at 5 seconds. 100 users × 10-second thread hold = 1,000 thread-seconds of wasted work.
...
| Time horizon | Designs for current stated requirements | Designs for current requirements and names the migration path when scale changes (10× rule) |
