...
Pre-compute and store each user's feed up to 1,000 items. Storage cost: approximately 1KB per feed item × 1,000 items × 100M users = 100 TB. At $0.023/GB for S3-compatible storage, that is $2,300/month. Read-time compute for the alternative (merge fan-out on read for all users) is much higher at peak.
...
Carry cost per month = (additional time per change in this area × change frequency per month) + (incident frequency per month × average incident resolution time × loaded hourly rate)

Payoff cost = engineering time to fix + migration risk cost (estimated probability of regression × cost of regression) + opportunity cost of not doing something else
...
Trade-off chosen: synchronous delivery, single database, no queue. Why acceptable: 10K users × 5 notifications/day = 50K notifications/day = less than 1 per second. A simple DB insert and a synchronous email call works fine.
