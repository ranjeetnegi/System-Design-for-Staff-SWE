...
Strong: "Let me be explicit about my assumptions. You said 10K users today. I'll design V1 for 100K users — that's the 10× horizon. At that scale, one database with read replicas is sufficient. I'll design the data model and API contracts so V2 (1M users) is a change to the infrastructure layer only, not the API. The things I'll NOT do today: sharding (not needed at 100K), Kafka (SQS is simpler and sufficient), multi-region (adds 3× operational complexity for a use case with no global requirement yet)."
Key insight: Over-engineering signals you don't understand the actual problem. Under-engineering signals you can't plan ahead. The right answer is 10× design with documented migration path to 100×.
...
flowchart TD
    ...
    S1 -->|Clarifies constraints,\nscopes appropriately| S1P[✓ Strong]
    S1 -->|Jumps to solution\nimmediately| S1F[✗ Weak: no L6 signal]
    S1P --> S2{Signal 2:\nTrade-off Articulation}
    S2 -->|Names options,\nchooses with reasoning| S2P[✓ Strong]
    S2 -->|Picks one approach\nwithout comparing| S2F[✗ Weak: no trade-off signal]
    S2P --> S3{Signal 3:\nAppropriate Depth}
    S3 -->|Breadth first,\nthen deep on hardest| S3P[✓ Strong]
    S3 -->|Deep on first\ncomponent, runs out of time| S3F[✗ Weak: tunnel vision]
    S3P --> S4{Signal 4:\nFailure Mode Awareness}
    S4 -->|Names failure modes\nnobody asked about| S4P[✓ Strong]
    S4 -->|Only designs\nhappy path| S4F[✗ Weak: no production awareness]
    S4P --> S5{Signal 5:\nOperational Maturity}
    S5 -->|Discusses monitoring,\nrunbooks, on-call| S5P[✓ Strong]
    S5 -->|"We'll add monitoring\nlater"| S5F[✗ Weak: no operational ownership]
    S5P --> S6{Signal 6:\nPractical Judgment}
    S6 -->|Designs for 10x,\ndocuments 100x path| S6P[✓ Strong]
    S6 -->|Designs for 1000x\nfrom day one| S6F[✗ Weak: over-engineering]
    S6P --> S7{Signal 7:\nCommunication Clarity}
    S7 -->|Maps the journey\nbefore hiking| S7P[✓ Strong → Hire signal]
    S7 -->|Monologue without\nstructure or check-ins| S7F[✗ Weak: unclear thinking]
```
...
Option A — Fan-out on write: when marketing sends a campaign, we immediately create 50M queue entries. Pro: each worker just reads from queue, no fan-out logic. Con: 50M queue writes = 50M × [message size] bytes. At 200 bytes each: 10GB just to enqueue one campaign.
...
Second: Our routing service is slow. Every notification is blocked on preference lookup. Fix: cache user preferences in Redis with 15-minute TTL. 50M users × 200 bytes = 10GB cache — affordable.
...
- [ ] I design for 10× scale, not 1000×
...
```
Volume × Unit cost = Monthly spend
Monthly spend × 12 = Annual spend
...
| "Poll every second for updates" | Never seen the compute invoice for 50M users × 86,400 seconds | "Push via WebSocket for active users; only poll on reconnect" |
...
| "Replicate to 3 regions for safety" | Does not know multi-region replication costs 3× storage + egress | "Replicate only the user-facing read path. Archive data stays in primary region." |
| "Use Spanner for all data" | Does not know Spanner costs 5–10× PostgreSQL | "Use Spanner for the consistency-critical path (preferences, invariants). Use cheaper storage for bulk history." |
