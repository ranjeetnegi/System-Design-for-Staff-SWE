...
- Derive peak QPS from average QPS and explain *why* peak is 3-5x average
...
Numbers also make trade-off discussions precise. "This adds 10 ms of latency" is more actionable than "this might be a bit slower." "This will cost $5,000 per month at current scale, reaching $50,000 at 10x growth" is more useful than "this could get expensive."
...
An estimate that is off by 2x is completely fine. Being off by 10x is usually still workable -- it affects server count but rarely changes the fundamental architecture. Being off by 1,000x (i.e., confusing thousands with millions) will completely break the design.

**What does "order of magnitude" mean **

An order of magnitude is a factor of 10. So:
- 100 and 1,000 are one order of magnitude apart (10x)
- 100 and 10,000 are two orders of magnitude apart (100x)
- 100 and 1,000,000 are four orders of magnitude apart (10,000x)
...
- 1M = 1,000 x 1K (a million is a thousand thousands)
- 1B = 1,000 x 1M = 1,000,000 x 1K (a billion is a thousand millions)
- 1T = 1,000 x 1B (a trillion is a thousand billions)

**Worked conversion drill** -- practice until automatic:

1. "300 million users" -> 0.3 billion -> 3 x 10 
...
| **1 Terabyte (TB)** | ~1.1 x 10   | ~1 trillion | A small data warehouse. 1,000 HD movies. The hard drive on a decent laptop. |
| **1 Petabyte (PB)** | ~1.1 x 10   | ~1 quadrillion | A large company's analytics data. YouTube stores petabytes of video. |
| **1 Exabyte (EB)** | ~1.1 x 10   | ~1 quintillion | Estimated total internet traffic in a few months. Google-scale storage. |
...
- 100M users x 200 bytes = 20,000,000,000 bytes = 20 GB
...
- 100M x 3 = 300M tweets/day

Step 2: How large is one tweet 
- tweet_id: 8 bytes
- user_id: 8 bytes
- text: 280 characters x 2 bytes (UTF-16) = 560 bytes, but avg is ~140 chars -> 280 bytes
- timestamp: 8 bytes
- metadata (likes, RT count, etc.): 50 bytes
- Total: roughly 354 bytes ~= 400 bytes (round up for overhead)

Step 3: Storage for one year:
- 300M tweets/day x 365 days = 109.5B tweets/year
- 109.5B x 400 bytes = 43.8 TB/year
...
|--------|--------------|-------|
| **Tweet (text only)** | 280 bytes | 280 chars x 1 byte (ASCII) or x2 (UTF-16) |
...
| 2^64 | ~1.8 x 10^19 | ~18 quintillion | Max int64; effectively unlimited for most ID spaces |
...
If you generate user IDs with a 64-bit integer, you have 2^64 ~= 18 quintillion possible values. At 1 million new users per day, you would run out in: 18 x 10^18 / (10^6 x 365) ~= 49 billion years. No need to worry about 64-bit IDs being exhausted.

**Practical drill: Can we fit N items in memory **

- 100M user IDs in memory (as int64): 100M x 8 bytes = 800 MB. Yes, one server.
- 1B user IDs: 1B x 8 bytes = 8 GB. One large server with 32+ GB RAM.
- 10B user IDs: 10B x 8 bytes = 80 GB. Requires a server with 128+ GB RAM, or multiple servers.
- 100B user IDs: 800 GB. Needs distributed storage.

The pattern: multiply count x bytes per item -> compare against server RAM size (typically 32-256 GB for cloud instances).
...
**Note on the QPS column:** This uses the formula: DAU x 20 actions/day / 86,400 ~= DAU x 0.00023. Adjust "actions per day" for your specific system. Social media feeds have higher action counts (30-50); payment systems have lower (1-5).
...
- **1K-10K QPS**: A read replica and cache gives you 10x headroom. Still manageable by a two-person team without heroics.
...
| L2 cache reference | 7 ns | Shared on-chip cache; 14x slower than L1 |
| Main memory (RAM) access | 100 ns | Off-chip; 200x slower than L1 |
| Compress 1 KB with Snappy | 3  s (3,000 ns) | CPU-bound |
| Read 1 MB sequentially from RAM | 9  s | Sequential access is fast |
| SSD random read (4 KB) | 16  s (16,000 ns) | Much faster than HDD; still 32,000x slower than L1 |
| Read 1 MB sequentially from SSD | 49  s | Fast sequential I/O |
| Round trip within same datacenter | 0.5 ms (500,000 ns) | Network in same AZ |
| HDD seek (random) | 2 ms (2,000,000 ns) | Mechanical head movement; 4,000,000x slower than L1 |
...
1. RAM vs SSD: **~1,000x slower** (100 ns vs 16  s -> 160x actually, but for sustained I/O the gap is larger)
2. SSD vs HDD: **~125x slower** for random reads (16  s vs 2 ms)
3. Local vs cross-continental network: **200,000x slower** than RAM (100 ns vs 150 ms)

**Practical design rule:** Every layer you move down in storage hierarchy costs you 100-10,000x. Design to keep hot data in RAM. Every database hit that could be a cache hit is hundreds of microseconds wasted. Every cross-region call that could be local is 100 ms wasted.
...
| SSD random read | 16  s -> 9 hours | A full work day |
...
Average latency: (990 x 20) + (9 x 200) + (1 x 5000) / 1000 = (19,800 + 1,800 + 5,000) / 1000 = 26.6 ms
...
- Users experiencing > 150 ms: 50,000 x 1% = 500 users/second = 30,000 users/minute
...
- P(all three <= 100 ms) = 0.99 x 0.99 x 0.99 = 0.970
...
- p50: 5 ms
- p99: 100 ms (20x worse than p50)
- p99.9: 500 ms (100x worse than p50)
...
- lambda (lambda) = arrival rate (requests per second = QPS)
...
- L = 500 x 0.020 = 10 requests in-flight on average
...
Peak is typically 4x average, so [peak QPS]. 
...
Once the order of magnitude is right, then refine. Getting the order of magnitude wrong early leads to architectures that are wrong by 1,000x. Precision within the right order of magnitude is a rounding error.
...
- Daily volume: 100M x 3 = 300M tweets/day
- Average QPS: 300M / 86,400 = 3,472 QPS
- Peak QPS (4x): 3,472 x 4 = **13,889 QPS ~= 14K write QPS**

**Reads (Tweet views):**
- Each user reads approximately 80 tweets/day (scrolling the feed)
- Daily volume: 100M x 80 = 8B reads/day
- Average QPS: 8B / 86,400 = 92,593 QPS
- Peak QPS (4x): 92,593 x 4 = **370,370 QPS ~= 370K read QPS**
...
- 300M tweets/day x 400 bytes/tweet = 120 GB/day
- Per year: 120 GB x 365 = **43.8 TB/year**
...
- Size: 10 min x 60 sec/min x 5 Mbps x (1/8) bytes/bit = 10 x 60 x 5/8 MB = 375 MB per video

**Transcoding multiplier:**
- YouTube transcodes to multiple resolutions: 4K, 1080p, 720p, 480p, 360p, 240p
- Each resolution is roughly proportional to pixel count; total storage for all resolutions ~= 2-3x the original
- Assume 2.5x factor: 375 MB x 2.5 = **937.5 MB ~= 1 GB per video**

**Annual storage:**
- 5M uploads/day x 1 GB/upload = 5 PB/day
- Per year: 5 PB x 365 = **1,825 PB = 1.825 EB/year**
...
- Daily data served: 5B x 10 min x 60 sec x 2 Mbps / 8 bytes = 75 x 10^15 bytes = 75 PB/day
...
**Message volume:**
- Write QPS: 1B x 50 = 50B messages/day / 86,400 = **578,703 messages/second average**
- Peak (5x because evenings are very concentrated): 578,703 x 5 = **2.9M messages/second**

**Fan-out (delivery operations):**
- Average group/conversation size: 2.5 recipients per message (mix of 1:1 and group chats)
- Delivery operations: 2.9M x 2.5 = **7.25M delivery operations/second at peak**
...
- 50B messages/day x 365 = 18.25T messages/year
- At 100 bytes/message: 18.25T x 100 = 1.825 PB/year
...
- 100M connections x 10 KB = 1 TB of RAM just for connection state -> requires thousands of servers dedicated to connection management
...
- Peak (2x because Google's traffic is more geographically spread than single-product apps): 57,870 x 2 = **115,740 QPS ~= 116K QPS**
...
**Total internal QPS:** 116K x 75 (median fan-out) = **8.7M internal operations/second at peak**
...
- Daily ride requests: 26M x 0.5 = 13M/day
- Average QPS: 13M / 86,400 = **150 QPS**
- Peak (3x -- rush hour): 150 x 3 = **450 QPS**
...
200,000 location updates/second dwarfs the 450 ride requests/second by 440x. The hard systems problem at Uber is not handling ride requests -- it is maintaining real-time geospatial state for 1 million moving vehicles.

**Storage for location history:**
- 200,000 updates/sec x 50 bytes/update (lat, lng, timestamp, driver_id) = 10 MB/sec
- Per day: 10 MB x 86,400 = 864 GB/day of raw GPS data
- With 2x replication: 1.7 TB/day
...
**Step 2 -- Input volume:**
- 1% of DAU posts each hour: 50M x 1% = 500,000 posts/hour
- Each post: ~200 bytes of metadata + hashtag list
- Input data per job: ~100 MB (500K posts x 200 bytes)

**Step 3 -- Output volume:**
- Top 100 hashtags + counts: 100 x (20 bytes hashtag + 8 bytes count) ~= 2.8 KB per output
- Per region (5 regions): 5 x 2.8 KB = 14 KB every 5 minutes -> trivial

**Step 4 -- Read path (serving trending):**
- At any 5-minute window, assume 30% of DAU are active: 50M x 30% = 15M users
...
**Practical rule:** Design for 2x expected peak. Use auto-scaling to right-size for valleys. Keep 50% headroom above sustained peak for burst absorption.

### 6.2 Availability vs Cost

Getting from 99.9% to 99.99% availability is not twice as hard -- it is 10x harder and typically costs 5-10x more infrastructure.

| Availability Target | What It Requires | Approximate Cost Multiplier |
|--------------------|-----------------|----------------------------|
| 99% | Basic setup, single-AZ | 1x |
| 99.9% | Multi-AZ deployment, health checks, runbooks | 2-3x |
| 99.99% | Multi-region active-active, auto-failover, chaos testing, 24/7 oncall | 5-10x |
| 99.999% | Custom hardware, custom software, full-time reliability team, zero-downtime deployments for everything | 20-50x |
...
**Insight:** The jump from 99.9% to 99.99% represents going from 8.76 hours of downtime per year to 52.6 minutes -- a reduction of 10x. But the engineering and operational effort does not scale linearly. It requires:
...
```
A_total = 0.999 x 0.999 x 0.999 x 0.9995 x 0.9995 x 0.999
A_total = 0.999  x 0.9995 
A_total ~= 0.997 x 0.999 = 0.996 ~= 99.6%
```
...
For a 99.9% SLO:
- Annual error budget: 0.1% x 525,600 minutes = **525.6 minutes per year**
...
**Step 1: Read QPS**
- Daily reads: 200M x 30 = 6B reads/day
- Average QPS: 6B / 86,400 = **69,444 QPS ~= 70K QPS**
- Peak QPS: 70K x 4 = **280K QPS**

**Step 2: Write QPS**
- Daily writes: 200M x 2 = 400M writes/day
- Average QPS: 400M / 86,400 = **4,629 QPS ~= 4.6K QPS**
- Peak QPS: 4.6K x 4 = **18.5K QPS**
...
- Posts per year: 400M posts/day x 365 = 146B posts/year
- Storage: 146B x 500 bytes = **73 TB/year**

**Step 5: Bandwidth**
- Read bandwidth: 70K QPS x 2 KB = **140 MB/s average**
- Peak read bandwidth: 280K x 2 KB = **560 MB/s**
...
- With 2x redundancy: **30 app servers**
...
**Database servers:**
- Cache handles 95% of reads -> DB sees: 5% x 280K = 14K read QPS
...
**Writes:**
- 100M x 5 = 500M short URLs created/day
- Write QPS: 500M / 86,400 = **5,787 QPS avg**
- Peak write QPS: 5,787 x 4 = **23,148 ~= 23K QPS**

**Reads (redirects):**
- 100M x 20 = 2B redirects/day
- Read QPS: 2B / 86,400 = **23,148 QPS avg**
- Peak read QPS: 23,148 x 4 = **92,592 ~= 93K QPS**

**Read:write ratio:** 93K : 23K ~= **4:1**

**Storage (5 years):**
- 500M x 365 x 5 = 912.5B short URLs
- Storage: 912.5B x 130 bytes = **118.6 TB** over 5 years
...
- 0.01% x 525,600 minutes = **52.6 minutes/year** of allowed downtime
...
2. **Zero-downtime deployments**: If each deployment takes 5 minutes of downtime, and you deploy twice a week, that is 5 x 2 x 52 = 520 minutes of planned downtime/year. You have blown your error budget in the first deployment. You need blue-green or canary deployments with no user-visible downtime.
...
- 2x infrastructure (two regions)
- 2x operational overhead (runbooks for two regions, chaos testing, more complex deployments)
- Dedicated reliability engineering team
- Roughly 5-10x the engineering effort compared to 99.9%
...
- 2B messages/day x 365 = 730B messages/year
- Storage: 730B x 100 bytes = **73 TB/year**

**Step 3: 2-year archive:**
- 73 TB x 2 = **146 TB** (before replication)
- With 3x replication: 146 x 3 = **438 TB**

**Step 4: Media messages (now add media):**
- Assume 15% of messages include photos (50 KB average)
- Photo messages: 2B/day x 15% = 300M/day x 50 KB = **15 TB/day** of media
- Per year: 15 TB x 365 = 5,475 TB = **5.5 PB/year** for media alone
...
**Naive estimate:** p99(checkout) ~= 6 x 50 ms = 300 ms
...
- P99(checkout) = 6 x 100 ms = 600 ms -> **SLO violated.**
...
- Driver location updates: **200,000 updates/second** -- 440x larger
...
- With 1 replica per shard (for HA): 34 x 2 = **68 nodes total**
...
- Load balancers: 2 x $25/month = $50/month
- CDN bandwidth: At 560 MB/s peak, assume average 200 MB/s sustained = 200 MB/s x 86,400 x 30 = 518 TB/month. CDN at $0.02/GB: 518,000 GB x $0.02 = $10,360/month
- Storage: 73 TB/year = 6 TB/month new storage. At $0.023/GB (S3): 6,000 x $0.023 = $138/month new + existing storage

**Rough total:** $3,000 + $600 + $1,200 + $50 + $10,360 + storage ~= **$15,000-$20,000/month**

**CDN dominates.** At scale, bandwidth cost is often larger than compute cost. This is why CDN optimization, image compression, and serving smaller payloads matters -- each KB saved x millions of requests = real money.

**At 10x scale (2B DAU equivalent):** Roughly $150K-200K/month. At this level, negotiating CDN contracts, building proprietary CDN infrastructure, and optimizing protocols (HTTP/3, Brotli compression) saves millions per year.
...
- Traffic peaks are typically 3-5x the daily average
- For a 24-hour period with a bell-shaped distribution:
  - Average is computed across all hours including quiet nighttime hours
  - Peak hour might be 4-8x the overnight minimum
  - The "peak to average" ratio for many web products is 3-5x
- For event-driven systems (news event, celebrity tweet, flash sale), peak can be 10-100x
- The standard assumption of 4x is a safe starting point unless you have reason to believe otherwise

**Full derivation:**
```
Average QPS = DAU x actions_per_user_per_day / 86,400 seconds/day
Peak QPS = Average QPS x peak_factor (default: 4)
```

**Example: 10M DAU, 20 actions per user per day:**
```
Average QPS = 10,000,000 x 20 / 86,400
            = 200,000,000 / 86,400
            = 2,314.8 QPS
            ~= 2,315 QPS

Peak QPS = 2,315 x 4 = 9,260 QPS ~= 9.3K QPS
```
...
- 100K events/sec x 200 bytes = **20 MB/sec ingestion rate**
- Per day: 20 MB x 86,400 = 1,728 GB = **1.73 TB/day**
- Per 90 days (hot tier): 1.73 x 90 = **155.7 TB**
...
- Concurrency: 5,000 x 0.1 sec average = 500 concurrent queries in-flight
...
- S3 for warm/cold: ~1 TB/day x 90 days = 90 TB at $0.023/GB = $2,070/month
...
```mermaid
flowchart TD
    A["Step 1<br/>Identify Core Entity<br/>What is being created/read/stored "] --> B["Step 2<br/>Estimate Volume<br/>DAU x actions/day = requests/day"]
    B --> C["Step 3<br/>Convert to QPS<br/>requests/day / 86,400 = avg QPS<br/>avg x 4 = peak QPS"]
    C --> D["Step 4<br/>Estimate Storage<br/>entities/day x size x retention_days"]
...
```
Average QPS = DAU x actions_per_day / 86,400
Peak QPS = Average QPS x 4  (3-5 depending on traffic pattern)
Storage = daily_entities x entity_size_bytes x retention_days
Servers = Peak_QPS / QPS_per_server x 2 (redundancy factor)
Availability (serial) = A  x A  x A  x ...
...
|---------|----------------|---------|
| **Forgetting peak factor** | System sized for 10K average; 40K peak crashes it | Always multiply by 3-5x for peak |
| **Ignoring fan-out** | "10K QPS" becomes 100K internal QPS with 10x amplification | Map the full call graph; size every node |
...
| **Assuming linear scaling** | "10 servers = 10x capacity" -- but overhead, coordination, and uneven distribution make it 7-8x | Apply efficiency factor; plan for rebalancing |
...
| **Ignoring growth** | Sized for today; growing by 3x in 18 months -> re-architect under load | Size for 2-3 years of growth |
| **Forgetting replication factor** | "100 TB storage" -> actually 200-300 TB with 2-3x replication | Multiply storage estimates by replication factor |
| **Averages hiding tails** | p50 latency looks fine; p99 is 10x worse -> SLO violated | Always instrument and alert on p95/p99 |
...
| SSD random read | 16  s | DB index lookup |
...
| **Peak traffic** | "We'll add servers if traffic spikes" | "Our traffic pattern shows 4x peak. We provision for peak plus 50% headroom. Auto-scaling handles the valleys and reduces cost by 30-40% compared to constant peak provisioning" |
| **Server count** | "We'll need a lot of servers" | "46K peak QPS, 20K per server -> 3 servers minimum. With 2x redundancy: 6 app servers. Cache absorbs 90% of reads, so DB sees 4.6K QPS -- 2 read replicas handle it" |
...
- "We have 10 million users" -> 10M x 20 actions / 86,400 ~= 2,300 QPS average. Peak ~9K. Two-tier architecture needed.
...
**Solution:**
- DAU: 500M x 30% = 150M DAU
- Daily reads: 150M x 50 = 7.5B reads/day
- Average QPS: 7.5B / 86,400 = **86,806 QPS ~= 87K QPS**
- Peak QPS (4x): 87K x 4 = **348K QPS**
- Annual storage: 7.5B reads/day x 1 KB/tweet x 365 (if we store each impression) = **2.74 PB/year**
  - Note: tweets themselves are stored once; feed impressions are ephemeral. Tweet storage: 150M DAU x 3 tweets/day x 365 x 1 KB = 164 TB/year (much smaller)

### Problem 2: Video Upload Storage
**Setup:** 10M video uploads per day, 100 MB average. Estimate storage for 90-day retention.

**Solution:**
- Daily storage: 10M x 100 MB = 1,000,000,000 MB = 1,000 TB = **1 PB/day**
- 90-day storage: 1 PB x 90 = **90 PB**
- With transcoding (2.5x for multiple resolutions): 90 x 2.5 = **225 PB**
- This requires distributed object storage (S3/GCS) with lifecycle policies

### Problem 3: Payment System Write QPS
**Setup:** 1M transactions per day, peak 5x. Each transaction does 3 DB writes.

**Solution:**
- Average QPS: 1M / 86,400 = **11.6 QPS**
- Peak QPS (5x): 11.6 x 5 = **58 QPS**
- DB write operations: 58 x 3 = **174 write ops/sec at peak**
...
**Solution:**
- Serial availability (both must work): 0.9995 x 0.9995 = 0.999 = **99.9%**
...
- Availability with active-active (either works): 1 - (1 - 0.9995)  = 1 - (0.0005)  = 1 - 0.00000025 = **99.99997%** [Y]
...
**Solution:**
- Total stored data (1B users x 5 GB average): 5 x 10^9 GB = **5 x 10^9 GB = 5 EB** (exabytes)
- New user uploads: 100K/day x 2 GB = 200 TB/day of net new data
- Existing user new uploads: assume 200M DAU x 10 MB/day new uploads = 2 PB/day
...
| RDS Multi-AZ (adds standby) | 2x compute cost | 2x above pricing |
...
- CDN (CloudFront): 560 MB/s peak, ~200 MB/s average = 200 MB/s x 86,400 x 30 / 1000 = 518 TB/month x $0.0085/GB = ~$4,400
- S3 storage: 73 TB/year = 6 TB/month new + accumulated = say 50 TB total x $0.023 = $1,150
...
**At 10x scale (500M DAU):** CDN scales linearly -> ~$44,000/month CDN alone. At this scale, companies negotiate enterprise CDN contracts (can reduce to $0.004-$0.007/GB) and invest in custom CDN infrastructure. The business case: at $44K/month vs $25K/month for a custom solution, it pays for itself within months.
...
Combined:
```
A_total = 0.9999 x 0.999999 x 0.9995 x 0.9995
        = 0.9999 x 0.999999 x 0.999 (approx: 0.9995 )
        ~= 0.9989
        ~= 99.89%
```
...
```
A_improved = 0.9999 x 0.999999 x 0.9999 x 0.9999 ~= 0.9997 ~= 99.97%
```
...
```
A_system = A_lb x A_api_pool x (A_cache + A_db - A_cache x A_db) ... 
```
...
```
A_system = 0.9999 x 0.999999 x 0.9995 ~= 0.9994 ~= 99.94%
```
...
```
A_critical = 0.9999 x 0.999999 x 0.9995 x 0.9995 ~= 0.9989 ~= 99.89%
```
...
- Notifications per day: 200M x 1 = 200M notifications/day
- Average QPS: 200M / 86,400 = 2,315 QPS
- Peak (3x): 6,944 QPS ~= **7K notification sends/sec**

**Fan-out to devices:**
- Each user has 2 devices average: 7K x 2 = **14K device pushes/sec**
...
- 200M x 365 x 50 bytes = **3.65 TB/year**
- Delivery receipts: 200M x 2 devices x 365 = 146B records/year x 30 bytes = **4.38 TB/year**
...
**Black Friday peak:**
- Orders: 10x normal = 10M orders/day = 120 QPS peak orders (but concentrated in 4 hours)
- In 4 hours: 10M / (4 x 3,600) = 694 orders/second at peak
- Product page traffic: 50x normal = 50 x 5,787 = 289K QPS
- Cart operations: 100x normal

**The spike is the challenge:**
- Normal: 12 QPS for orders -> trivially handled by a single DB
- Black Friday peak: 694 QPS for orders -> 58x normal, still manageable by a good DB with write optimization
- Product pages: 289K QPS at peak -> 50x normal -> cache is essential, CDN for static assets

**Auto-scaling plan:**
- 48 hours before Black Friday: pre-scale to 3x normal capacity (known spike)
- During event: auto-scale up to 10x based on CPU/QPS metrics
- Post-event: scale down over 12 hours to avoid cold-start latency from sudden scale-in

**Key numbers to design around:**
- 694 orders/second x 5 DB writes per order = 3,470 DB writes/second at order peak
...
**Write QPS:**
- 5M users x 1 score update/sec = **5M writes/second**
...
**Storage:**
- 5M user scores x 16 bytes (int64 user_id + int64 score) = 80 MB -- tiny
...
**At 10K user-facing QPS:** 610K internal operations/second. If you sized all services for 10K QPS, they are all under-provisioned by 61x.
...
- Effective capacity scaling is typically 7-8x per 10x server increase (not 10x)
...
- Add redundancy: 14 x 2 = **28 servers**
...
**Correct estimate:** "We need 100 TB of logical storage. With 3x replication, that is 300 TB of physical storage."
...
- 1M concurrent users x 1 request per 5 seconds = **200K QPS** (not 1M)
...
- Convert as needed: concurrent_connections x requests_per_connection_per_second = QPS
...
**Examples:**
- **LSM tree (Cassandra, RocksDB):** Writes go to memtable first, then get compacted to SSTable levels. A logical write of 1 KB can result in 5-10 KB written to disk over the compaction lifecycle. Write amplification factor: 5-10x.
- **Replication:** Writing 1 record that must be replicated to 3 nodes -> 3 physical writes.
- **WAL (Write-Ahead Log):** Databases write to WAL before the actual data file. Effective write amplification ~= 2x.
- **Combined:** Cassandra with 3 replicas and 5x compaction amplification: 15x actual writes per logical write.

**Impact on QPS calculation:**
- If you need 10K logical write QPS with Cassandra (3 replicas, 5x compaction): storage layer sees 150K IOPS
...
Math: 6 x 200 ms = 1,200 ms from DB alone. Add network overhead, serialization, and middleware: total p99 hit 2,500 ms.
...
- 100 requests/sec x 60 seconds = 6,000 users per minute experiencing a timeout
...
P(all three finish <= 100 ms) = 0.99 x 0.99 x 0.99 = 0.99  ~= 0.970
```
...
```
130M MAU x 20% DAU = 26M DAU
26M DAU x 0.5 rides per day = 13M ride requests per day
13M / 86,400 = ~150 QPS average
Peak (3x for rush hour): 150 x 3 = 450 ride request QPS
```
...
Each ride request triggers several internal operations: matchmaking, ETA calculation, pricing, push notifications. Roughly 10x internal fan-out.

```
450 QPS x 10 = 4,500 internal QPS at peak
```
...
Driver location updates are **220x to 550x larger** than the primary user action.
...
- Heartbeat/presence updates (chat apps) -- often 10-100x message volume
- Analytics event streams -- often 50-100x API request volume
...
```
Annual downtime budget = 0.1% x 525,600 minutes = 525.6 minutes/year
...
```
A_total = 0.999 x 0.999 x 0.999 x 0.9995 x 0.999 x 0.999

Step 1: 0.999^5 = 0.999 x 0.999 x 0.999 x 0.999 x 0.999
       = 0.995009... ~= 0.9950

Step 2: 0.9950 x 0.9995 = 0.9945
```
...
```
(1 - 0.9945) x 525,600 minutes = 0.0055 x 525,600 = 2,890 minutes ~= 48 hours/year
```
...
P(both fail simultaneously) = 0.001 x 0.001 = 0.000001
...
The formula P(both fail) = P(A fails) x P(B fails) only holds when the two failures are **independent**. In practice, this requires different failure domains:
...
```
A(parallel but both required) = A_auth x A_userservice
                               = 0.999 x 0.999
                               = 0.998 = 99.8%
```
...
```
A_total = A_api x A_parallel x A_db
        = 0.9995 x 0.998 x 0.9995
        = 0.997 ~= 99.7%
```
...
Architecture:
- 20 app servers x $140/month = $2,800
- 2 Redis nodes x $150/month = $300
- 1 DB primary x $400/month = $400
- 2 DB replicas x $400/month = $800
- 1 load balancer x $30/month = $30
- Bandwidth and storage ~= $500/month

Total: approximately **$4,830/month ~= $5,000/month**

At 100K QPS: $5,000 / 100,000 QPS / (30 days x 86,400 sec/day) ~= **$0.02 per 1,000 requests**
...
3. **Projecting growth costs**: "We are at $5,000/month now. At 10x growth (which our product team targets in 18 months), we will be at $50,000/month. We should negotiate reserved instances now to reduce that to $30,000/month."
...
Step 1: DAU
```
500M x 0.30 = 150M DAU
```

Step 2: Daily reads
```
150M x 50 tweets/day = 7,500,000,000 = 7.5B reads/day
```
...
Step 4: Peak read QPS (4x factor for social apps)
```
87K x 4 = 348K QPS at peak
```

Step 5: Storage -- tweet impressions logged (if you log every view)
```
7.5B reads/day x 1 KB x 365 days = 2.74 PB/year of impression logs
```
...
7.5M x 1 KB x 365 = 2.74 TB/year of tweet text
```
...
```
10M uploads x 100 MB = 1,000,000,000 MB = 1,000 TB = 1 PB per day
```

Step 2: 90-day storage before compression
```
1 PB/day x 90 days = 90 PB
```
...
```
115 uploads/sec x 10 seconds/upload = 1,150 concurrent in-flight uploads
```
...
**Setup:** 1M transactions per day, peak 5x, each transaction requires 3 DB writes.
...
Step 2: Peak TPS
```
11.6 x 5 = 58 TPS at peak
```

Step 3: DB write operations at peak
```
58 TPS x 3 writes/transaction = 174 write QPS
```
...
```
1M transactions/day x 365 days = 365M transactions/year
365M x 500 bytes/transaction = 182.5 GB/year
```
...
P(both fail simultaneously) = 0.0005 x 0.0005 = 0.00000025
...
```
4 x 30 seconds = 120 seconds = 2 minutes of failover downtime
```
...
```
12 x 5 minutes = 60 minutes > 52.6 minutes (budget exceeded)
```
...
*Same problem at four levels: "How much storage does Twitter need for tweets?"*

### Intern Level: "Count the tweets"

The intern thinks: "Twitter has 500M tweets/day. A tweet is 280 characters. 280 bytes per tweet. 500M x 280 = 140GB/day."

Think of this like calculating how much space a library needs by counting the words in books, forgetting the shelves, the building, the parking lot, and the staff offices.

The intern's 140GB/day is just the raw text. They forgot:
- Metadata: user_id, timestamp, reply_to, retweet_of, location = 5x the tweet size
- Media: photos, videos attached to tweets (the vast majority of Twitter's actual storage)
- Indexes: to find tweets by user, by hashtag, by recency = another 2-3x
- Replication: 3x for durability
- Archival: old tweets are kept, not deleted

Real answer is 10-50x higher. The intern got the right units (GB) but the wrong magnitude.

### Mid-Level (L4): "Add all the components"

L4 builds a more complete model:
- Tweet text: 280 bytes x 500M = 140GB/day
- Metadata: 200 bytes x 500M = 100GB/day
- Media: 30% of tweets have images (avg 500KB), 5% have videos (avg 5MB)
  - Images: 0.3 x 500M x 500KB = 75TB/day
  - Videos: 0.05 x 500M x 5MB = 125TB/day
- Total: ~200TB/day raw

Better. But L4 stopped at raw storage. They didn't account for: CDN caching (video watched 1,000 times = stored once, served 1,000 times), encoding overhead (video in 5 resolutions), or whether "per day" means "net new" vs "served" (the same tweet is read 1,000 times, not stored 1,000 times).

### Senior (L5): "Separate storage from bandwidth, estimate with confidence intervals"

L5 distinguishes three separate quantities that L4 conflated:
1. Write storage: new data created per day (L4's 200TB)
2. Total storage: 5 years of data retained = 200TB x 365 x 5 = 365PB (petabytes)
3. Read bandwidth: how much data is served per day = (total reads x avg size per read)

L5 estimates: Twitter serves 300B timeline views/day. Each timeline = ~20 tweets shown, average 100KB including thumbnails = 300B x 20 x 100KB = 600PB of bandwidth served per day. This is why CDNs exist: without caching, Twitter would need 600PB/day of origin bandwidth.

L5 also adds confidence intervals: "My tweet storage estimate has 3x uncertainty (media attachment rates vary, video resolution varies). My bandwidth estimate has 5x uncertainty. When presenting, I'll say '200TB/day +/- 3x' not '200TB/day' like I have 3 significant figures of precision."

```
L5 ESTIMATION FRAMEWORK:
  1. Separate: writes vs reads vs storage vs bandwidth
  2. Identify the dominant term (media >> text for storage)
  3. Estimate dominant term carefully, rough the rest
  4. Add replication multiplier (3x), index multiplier (2x)
  5. Express as range, not a point estimate
```

### Staff (L6): "The estimate is a product decision, not a math exercise"

L6 does the math then asks: "What decisions does this estimate drive?"

"Our 365PB total storage estimate implies a $73M/year S3 bill ($0.023/GB x 365PB). That's 40% of our infrastructure budget. Decision: do we delete tweets older than 7 years? Do we compress cold data with Zstandard (10:1 ratio -> $7.3M/year)? Do we negotiate enterprise S3 pricing?"

"Our 600PB/day bandwidth estimate implies $54M/year in CDN egress at $0.09/GB. Decision: self-host CDN? Negotiate Cloudflare enterprise contract? The estimate drives a $50M negotiation."

L6 also validates the estimate: "Let's sanity check: Amazon S3 serves 100 trillion objects total. Twitter has ~5T tweets. Our 365PB estimate seems plausible. Alternatively: YouTube (similar media scale) uses 1EB total storage, reported publicly. Twitter at 10% of YouTube's scale = 100PB. Our estimate is in the right ballpark."

```
L6 ESTIMATION ADDITIONS:
  - Translate storage to cost ($)
  - Identify the decisions the estimate drives
  - Sanity-check against public benchmarks
  - Present uncertainty explicitly
  - Show what reduces cost (compression, tiering, deletion policy)
```

### The Pattern

- Intern: estimates raw text only, misses media and overhead
- L4: adds all components, misses the write/read/bandwidth distinction
- L5: separates writes/storage/bandwidth, adds confidence intervals
- L6: translates to cost, drives product decisions, sanity-checks against industry benchmarks

---

## L5 vs L6 Calibration: Numbers and Estimation

| Dimension | L5 (Senior) | L6 (Staff) |
|-----------|-------------|------------|
| Estimation approach | Bottom-up: estimate each component, sum | Top-down + bottom-up: cross-check both, use the tighter bound |
| Media overhead | Includes image/video in storage | Distinguishes storage at rest vs bandwidth served (CDN caching) |
| Replication factor | Applies 3x replication to storage | Applies per-tier replication (hot: 3x, warm: 2x, cold: 1x) |
| Confidence interval | "Roughly" or "around" | Explicit +/- N multiplier: "200TB +/- 3x" |
| Cost translation | Can estimate cost if prompted | Always translates to cost, surfaces $-impact of design decisions |
| Sanity check | Checks order of magnitude | Cross-checks against 2+ public benchmarks (YouTube, Amazon, Facebook reports) |
| Dominant term | Finds it | Finds it AND explains why other terms are negligible |
| Latency numbers | Knows the classic table (L1 cache: 1ns, etc.) | Uses latency numbers to design: "at 160ms round-trip, we need edge caching" |
| QPS estimation | Computes QPS from DAU x events/day | Computes peak QPS (10x average) and designs for peak, not average |
| Storage tiering | Knows hot/warm/cold exist | Designs tiering policy: age-based, access-frequency-based, with cost model |
| Growth modeling | Estimates today's numbers | Projects 12-month growth, identifies when you hit the next architectural threshold |
| Presentation | Gives a number | Gives a range, lists assumptions, and states which assumption changes the answer most |
...
### Incident 1: Twitter 2022 -- Write Load Estimation Miss Post-Acquisition

**What happened:** After Elon Musk's acquisition of Twitter, the infrastructure team decommissioned a large portion of servers based on estimates that the remaining fleet could handle 50% of peak load. Tweet delivery latency spiked to 30+ seconds for users with large follower counts almost immediately. The system buckled under what should have been normal write traffic.

**Root cause:** The team modeled fan-out write amplification at 50x per tweet (one write fans out to 50 follower timelines). The real number for top accounts was 200x. Every estimate downstream of that number -- servers needed, queue depth, storage writes per second -- was 4x too small.

**ASCII diagram:**
```
MODELED:
  1 tweet write --> [fan-out worker] --> 50 timeline writes
  QPS model:  10,000 tweets/sec x 50  =  500,000 writes/sec  <-- sized for this

ACTUAL:
  1 tweet write --> [fan-out worker] --> 200 timeline writes
  Real QPS:   10,000 tweets/sec x 200 = 2,000,000 writes/sec  <-- 4x over capacity
```

**Fix applied:** Twitter emergency-rolled back the decommissioning and re-provisioned servers, then rate-limited fan-out for accounts above a follower threshold until capacity could be rebuilt.

**Staff lesson:** L6 engineers never model write amplification as a single average. They ask: what is the p99 amplification factor, and what happens to the system if top-1% accounts all post at the same time? Fan-out amplification must be stress-tested at the extreme tail, not just the mean.

---

### Incident 2: Reddit 2015 -- Database Storage Runway Missed

**What happened:** Reddit's primary PostgreSQL database hit full storage with only 4 hours of warning. The team had to perform an emergency migration under live traffic, with real risk of data loss during the scramble.

**Root cause:** The infrastructure model projected database growth at 50GB/month based on historical write patterns. A new "save post" feature that launched 6 months earlier was never added to the capacity model. The actual growth rate was 200GB/month -- 4x higher. The team had done the estimate once at launch and never revisited it.

**ASCII diagram:**
```
MODELED (done at launch, never updated):
  month 1:  50GB
  month 6: 300GB  <-- runway looked fine

ACTUAL (save-post feature not included):
  month 1:  200GB
  month 6: 1,200GB  <-- hit the disk limit
  
  gap = 900GB of unmodeled storage over 6 months
```

**Fix applied:** Reddit migrated to a larger database instance under live traffic and added automated storage growth alerts that fire when projected runway drops below 30 days.

**Staff lesson:** L6 engineers treat capacity models as living documents. Any new feature that changes write volume must trigger a capacity model update before launch. A single estimate done at project start is not a capacity plan -- it is a starting guess.

---

### Incident 3: WhatsApp 2021 -- Bandwidth Spike During Privacy Policy Panic

**What happened:** WhatsApp updated their privacy policy in January 2021. Millions of users panicked and simultaneously triggered the GDPR data export feature to download their personal data archives. CDN egress costs for that week alone exceeded the entire month's budget. Origin servers became overwhelmed serving data exports that had not been cached.

**Root cause:** The bandwidth model assumed a steady-state download rate of 0.01% of daily active users per day using the data export feature. The panic-driven spike was 8% of daily active users -- 800x the modeled rate. The CDN layer had no warm cache for these exports because demand had always been too low to cache proactively.

**ASCII diagram:**
```
MODELED:
  DAU = 500M
  data-export users/day = 0.01% x 500M = 50,000 users
  avg archive size = 500MB
  egress/day = 50,000 x 500MB = 25TB/day  <-- CDN budgeted for this

ACTUAL (panic spike):
  data-export users/day = 8% x 500M = 40,000,000 users
  egress/day = 40M x 500MB = 20,000TB/day = 20PB/day  <-- 800x over
```

**Fix applied:** WhatsApp pre-generated and cached data exports on a rolling basis so CDNs could serve them without hitting origin, and added rate limiting on export requests per user per day.

**Staff lesson:** L6 engineers ask: what is the worst-case demand for every low-frequency feature if a correlated external event triggers everyone to use it at once? Features with "normally quiet" traffic are the most dangerous blind spots in capacity models.

---

### Incident 4: Zoom 2020 -- QPS Capacity Model Shattered by COVID Lockdowns

**What happened:** Zoom's daily active users grew from 10 million to 200 million in one month during March 2020 COVID lockdowns -- a 20x spike in 30 days. WebRTC signaling servers that coordinate call setup hit their capacity limits first. Call join failure rates reached 15% during peak hours, making Zoom unreliable at exactly the moment the world needed it most.

**Root cause:** Zoom's QPS capacity model was built with 3x-5x headroom above current observed load. That headroom is enough to survive normal spikes and seasonal traffic. It is not designed to survive a 20x jump in 30 days. Hardware procurement and cloud capacity reservations typically take 3+ months to fulfill, so Zoom could not respond fast enough.

**ASCII diagram:**
```
CAPACITY MODEL (3x-5x headroom):
  current load:   10M DAU --> 100,000 signaling QPS
  headroom limit: 300,000 - 500,000 signaling QPS  <-- model said safe

ACTUAL GROWTH:
  March 1:   10M DAU  --> 100,000 QPS
  March 10:  50M DAU  --> 500,000 QPS  <-- hit model ceiling
  March 31: 200M DAU  --> 2,000,000 QPS  <-- 20x over ceiling
  
  failure started at day 10, not day 30
```

**Fix applied:** Zoom made emergency cloud capacity reservations and shifted signaling load to cloud instances while permanent hardware was procured. They also started publishing capacity forecasts internally on a weekly basis instead of quarterly.

**Staff lesson:** L6 engineers distinguish between headroom for normal variance (3x-5x is fine) and headroom for catastrophic demand shifts. For any product that can go viral or become essential during a crisis, you need a separate "emergency scale plan" that can activate within days, not months.

---

### Incident 5: Stack Overflow 2017 -- Health Check Removed Servers During Overload

**What happened:** A viral Reddit thread linked directly to a Stack Overflow page and drove 10x normal traffic in under 10 minutes. The load balancer's health checks began timing out on web servers that were slow but still functional. The load balancer removed those servers from the active pool -- which put more load on fewer servers -- which caused more health check failures. The cascade made the overload significantly worse than it needed to be.

**Root cause:** The capacity plan correctly sized for average and expected peak QPS based on measured traffic. It did not model the failure mode where the load balancer itself removes capacity during overload. The health check timeout was tuned for normal latency (200ms), not overloaded latency (2,000ms). A server responding slowly is not a dead server, but the configuration could not tell the difference.

**ASCII diagram:**
```
NORMAL:
  [Load Balancer] --> 11 web servers, each healthy
  traffic: 1x QPS, all health checks pass

OVERLOAD START:
  traffic: 10x QPS
  server response time: 200ms --> 2,000ms
  health check timeout: 200ms --> FAIL

CASCADE:
  step 1: LB removes 4 slow servers (they are still alive)
  step 2: 10x traffic now hits 7 servers instead of 11
  step 3: 7 servers slow down more --> more health check failures
  step 4: LB removes 3 more servers
  step 5: 10x traffic now hits 4 servers  <-- site down
  
  the load balancer made the problem 2.5x worse
```

**Fix applied:** Stack Overflow raised health check timeouts during high-traffic periods and added a rule that prevents the load balancer from removing more than 20% of the pool within any 60-second window, even if health checks fail.

**Staff lesson:** L6 engineers design for the failure modes that appear *during* overload, not just the steady-state capacity numbers. The question is not just "can the system handle 10x QPS?" -- it is "what does the system do to itself when 10x QPS arrives?" Automated systems like health checkers and auto-scalers can amplify failures if their thresholds are not designed with overload in mind.

---

## Supplemental Brainstorming Questions

**Question 13:** A product manager tells you "we expect 1 million users at launch." What 5 follow-up questions do you ask before estimating QPS?

**Question 14:** Your read QPS is 50,000/second. Each read touches 3 microservices. What is the total QPS load across your system? What changes if service B fans out to 2 downstream services per request?

**Question 15:** A database table is at 10TB and growing 200GB/month. How many months before you hit 20TB? At what point should you start planning a migration (6 months out? 12 months?), and what triggers the decision?

**Question 16:** You have 100M active users. 1% upload a photo per day. Average photo size: 2MB (original), 200KB (thumbnail), 50KB (thumbnail for mobile). How much total storage is written per day? Per year? At 3x replication?

**Question 17:** Your API gateway logs 5M requests/day. Average response size: 10KB. How much bandwidth do you serve per day? Per month? At what point does CDN caching (80% hit rate) change that number?

**Question 18:** A service has p50 latency of 20ms and p99 of 200ms. If 5 such services are called in serial, what is the expected p99 of the full request? What about in parallel?

**Question 19:** Your team estimates the system needs 50 servers. Your manager asks for a 70% cost reduction. What levers do you pull first? What is the cost of each lever (in engineering time vs dollars saved)?

**Question 20:** Twitter has 500M registered users and 200M daily active users. If DAU is 40% of registered users, and you are building a similar product that just hit 10M registered users, what is a realistic DAU estimate? What assumption is most likely wrong?

**Question 21:** A new feature sends a push notification to all users when a trending post hits 1M views. At 100M users and 10 trending posts/day, what QPS does your push notification service receive? (Show the math step by step.)

**Question 22:** You need 4 nines of availability (99.99%). Your deploy pipeline takes 10 minutes to roll back. How many incidents of exactly 10 minutes of downtime can you tolerate per year? (Show the math.)

**Question 23:** A video streaming service serves 4K video at 15 Mbps. At 100,000 concurrent viewers, how much bandwidth must the CDN deliver? If the CDN charges $0.09/GB, what is the cost per hour?

**Question 24:** Your PostgreSQL table has 500M rows, growing at 2M rows/day. An index on `created_at` is 40GB. If you add a composite index on `(user_id, created_at)`, estimate its size. At what table size does an index full scan become slower than a full table scan?
