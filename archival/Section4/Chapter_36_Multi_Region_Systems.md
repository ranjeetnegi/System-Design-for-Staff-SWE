...
# Appendix: RTO/RPO in Multi-Region Context

Multi-region architecture is often motivated by DR requirements. RTO and RPO targets directly determine the multi-region topology.

## Multi-Region Topology ↔ RTO/RPO Mapping

```
┌─────────────────────────────────────────────────────────────────────────────┐
│   RTO/RPO → MULTI-REGION TOPOLOGY                                           │
│                                                                             │
│   RPO=0, RTO<1m:                                                            │
│   ┌──────────────────────────────────────────────────────────────────┐      │
│   │  ACTIVE-ACTIVE: Both regions serve traffic. Sync replication.    │      │
│   │  Write to both → conflict resolution needed.                     │      │
│   │  Failover: automatic, no data loss. Cost: 2× infrastructure.   │      │
│   └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│   RPO<5m, RTO<15m:                                                          │
│   ┌──────────────────────────────────────────────────────────────────┐      │
│   │  ACTIVE-PASSIVE with ASYNC REPLICATION:                          │      │
│   │  Primary serves traffic. Secondary receives async replication.   │      │
│   │  Failover: DNS switch (2-5 min) + replication lag (up to 5 min).│      │
│   │  Data loss: up to replication lag at time of failure.            │      │
│   └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│   RPO<1h, RTO<4h:                                                           │
│   ┌──────────────────────────────────────────────────────────────────┐      │
│   │  WARM STANDBY: Secondary has infrastructure ready, data lagging. │      │
│   │  Restore from snapshots. Manual or semi-automated failover.      │      │
│   │  Acceptable for non-user-facing or internal systems.            │      │
│   └──────────────────────────────────────────────────────────────────┘      │
│                                                                             │
│   STAFF INSIGHT: Most systems don't need RPO=0. The cost difference        │
│   between RPO=0 (sync replication) and RPO=5m (async) is often 3-5×.       │
│   Quantify the business cost of data loss vs infrastructure cost.          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

## The Hidden Components of RTO

```
┌─────────────────────────────────────────────────────────────────────────────┐
│   RTO IS NOT JUST "PROMOTE THE REPLICA"                                      │
│                                                                             │
│   Your actual RTO includes ALL of these:                                    │
│                                                                             │
│   ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐       │
│   │ Detect   │ │ Decide   │ │ Failover │ │ Validate │ │ Warm up  │       │
│   │ failure  │→│ to fail  │→│ (DNS,    │→│ data     │→│ caches,  │       │
│   │          │ │ over     │ │ promote  │ │ integrity│ │ drain    │       │
│   │ 1-5 min  │ │ 2-10 min │ │ replica) │ │          │ │ backlogs │       │
│   │ (alerts) │ │ (human   │ │ 2-15 min │ │ 5-30 min │ │ 5-60 min │       │
│   │          │ │ decision)│ │          │ │          │ │          │       │
│   └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────┘       │
│                                                                             │
│   Documented RTO: 15 minutes ("promote replica and switch DNS")            │
│   Actual RTO: 15 + 10 + 30 + 60 = 115 minutes                             │
│                                                                             │
│   STAFF RULE: Measure ACTUAL RTO including detection, decision,            │
│   validation, and warm-up. Most teams' real RTO is 5-10× their target.   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

## DR Testing in Multi-Region

| Test Type | Frequency | What It Validates |
|-----------|-----------|-------------------|
| **Backup restore** | Monthly | Can we restore from backup within RPO? |
| **Failover drill** | Quarterly | Can we failover within RTO? Is data consistent? |
| **Chaos test** | Monthly | Kill primary region; does secondary take over? |
| **Data validation** | Continuous | Is secondary region's data within RPO of primary? |
| **Runbook review** | Quarterly | Are DR runbooks accurate and up-to-date? |

**Staff principle**: An untested DR plan is not a DR plan. If you've never failed over, you don't know your actual RTO—only your hoped-for RTO.

**One-liner**: "Your documented RTO is a fantasy. Your measured RTO is a fact. Run the drill."
