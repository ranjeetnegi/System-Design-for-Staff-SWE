...
After a TCP connection is closed, the port enters **TIME_WAIT** state. The OS holds the port for 2 × MSL (Maximum Segment Lifetime) — typically 60–120 seconds. This prevents delayed packets from a dead connection confusing a new connection.
...
- Ports occupied simultaneously = 100 × 240 = **24,000 ports**
...
sequenceDiagram
    ...
    A->>B: ACK (ack=Y+1)
    Note over A: State: TIME_WAIT (waits 2×MSL before port released)
    Note over B: State: CLOSED

    Note over A: After 2×MSL (~60–120 seconds): CLOSED
```

### TIME_WAIT

The side that **initiates** close enters TIME_WAIT after sending the final ACK. It stays there for **2 × MSL** (Maximum Segment Lifetime), typically 60–120 seconds.
...
- **TLS 1.2:** 2 RTT × 200ms = **400ms** before first data
- **TLS 1.3:** 1 RTT × 200ms = **200ms** before first data
- **Savings:** 200ms per new connection
...
flowchart TD
    ...
    Q4 -->|"No"| S5["Step 5: Check DB max_connections<br/>instances × pool_size vs DB limit"]
...
If queries are slow (say, 2 seconds instead of 5ms), each connection is held for 400x longer than normal. Little's Law: at 100 QPS × 2s = 200 simultaneous connections. If pool is 50, it exhausts in seconds. Pool exhaustion is the *symptom*; slow queries are the *cause*. Fix: optimize queries AND scale the DB. Increasing pool size alone just shifts the exhaustion point.
...
The database has its own connection limit. Example: 500 connections per app instance × 10 instances = 5,000 connections, but DB `max_connections = 100`. Result: 4,900 connections rejected. Use **PgBouncer** to multiplex: 10 app instances × 500 pool connections = 5,000 client connections to PgBouncer → PgBouncer maps these to 50 real PostgreSQL connections.
...
- In-flight requests holding connections: 1,000 QPS × 2s = **2,000 requests in flight**
- Pool size: 200
